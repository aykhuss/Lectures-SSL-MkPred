...
** Comparison to data
There is an ATLAS measurement [[https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/STDM-2018-05/][2309.09318]] that is inclusive in the lepton kinematics and thus suitable for performing a simple comparison here.
...
The paper is published and the associates data is available on [[https://www.hepdata.net/record/ins2698794][HEPData]].
However, it appears that HEPData is missing the first rapidity bin ([0-0.4]) but we can also just use tools like [[https://arxiv.org/abs/0710.2896][EasyNData]] that allows us to extract data point out of plots :)
